...
class UnauthorizedEmailProviders implements ValidationRule
{
     ...
     */
    private ?array $providerLookup = null;
    ...
    public function validate(string $attribute, mixed $value, Closure $fail): void
    {
        ...
        if (mb_strpos($value, '@') !== false) {
            ...
            if ($this->hasUnauthorizedProvider($emailProvider)) {
                $fail('Le :attribute utilise un fournisseur d’email non autorisé.');

                return;
            }
        } else {
            $fail('Le champ :attribute doit contenir une adresse email valide.');

            return;
        }
    }

    private function hasUnauthorizedProvider(string $emailProvider): bool
    {
        $this->providerLookup ??= array_fill_keys(
            array_map(
                static fn(string $provider): string => mb_strtolower(trim($provider)),
                $this->unauthorizedEmailProviders
            ),
            true
        );

        return isset($this->providerLookup[$emailProvider]);
    }
}
